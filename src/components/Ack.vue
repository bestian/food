  ...
  .ui.grid.container
    .ui.doubling.two.column.row
      ...
      .column
        ...
        br
        .ui.vertical.buttons
          ShareNetwork.ui.huge.blue.button(network="facebook", :url="'https://food.bestian.tw/#/ack/' + $route.params.id", title="自然美食DIY", :description="'自然美食DIY - ' + pop.t", :quote="'簡單又美好的食譜 - ' + pop.t", @click="trackButton('share', 5)")
            i.share.icon
            | 分享此頁&nbsp;
            i.facebook.icon
          router-link(to="/snacks", v-if="check()").ui.huge.green.button
            i.arrow.left.icon
            | 回上一頁&nbsp;
            i.leaf.icon
...
export default {
  ...
  methods: {
    ...
    save() {
      ...
      fallbackCopyTextToClipboard('https://food.bestian.tw/#/ack/' + this.$route.params.id);
      // eslint-disable-next-line
      this.show('本頁連結為: https://food.bestian.tw/#/ack/' + this.$route.params.id);
    },
    has(j, k) {
      if (!k) return true;
